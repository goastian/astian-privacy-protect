...
echo "*** midori-protection.mv3: Creating extension"
...
UBOL_DIR="dist/build/midori-protection.mv3.$PLATFORM"
...
echo "*** midori-protection.mv3: Copying common files"
...
echo "*** midori-protection.mv3: Copying mv3-specific files"
...
echo "*** midori-protection.mv3: Generating rulesets"
...
echo "*** midori-protection.mv3.$PLATFORM: extension ready"
...
if [ -z "$TAGNAME" ]; then
    ...
    if [ "$PLATFORM" = "firefox" ]; then
        jq '.browser_specific_settings.gecko.id = "midori-protection@astian.org"' "$UBOL_DIR/manifest.json"  > "$tmp_manifest" \
            && mv "$tmp_manifest" "$UBOL_DIR/manifest.json"
    fi
else
...
if [ "$PLATFORM" = "edge" ]; then
    ...
    echo "*** midori-protection.mv3.edge: Modify reference implementation for Edge compatibility"
...
if [ "$FULL" = "yes" ]; then
    ...
    echo "*** midori-protection.mv3: Creating publishable package..."
    UBOL_PACKAGE_NAME="midori-protection-mv3_$TAGNAME.$PLATFORM.$EXTENSION"
