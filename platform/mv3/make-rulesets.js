...
async function fetchListFromCache(assetDetails) {
    ...
    if ( assetDetails.enabled === false ) {
        log(`!!! Skipping unavailable optional list: ${assetDetails.id}`, false);
        return '';
    }

    throw new Error(`Required filter list is empty: ${assetDetails.id}`);
}
