...
import {
    getConsoleOutput,
    getMatchedRules,
    getTabActivity as getTrackedTabActivity,
    isSideloaded,
    toggleDeveloperMode,
    ubolErr,
    ubolLog,
...
async function getPopupActivity(tabId) {
    const activity = await getTrackedTabActivity(tabId);
    const badgeText = await browser.action.getBadgeText({ tabId })
        .catch(( ) => '');
    const nativeBadgeCount = Number.parseInt(badgeText, 10) || 0;
    if ( activity.total === 0 && nativeBadgeCount !== 0 ) {
        activity.total = nativeBadgeCount;
        activity.other = nativeBadgeCount;
    }
    return activity;
}
...
async function onMessage(request, sender) {
    ...
    case 'popupPanelData': {
        const results = await Promise.all([
            ...
            getPopupActivity(request.tabId),
        ]);
    ...
    case 'getTabActivity': {
        return getPopupActivity(request.tabId);
    }
...
async function start() {
    ...
    // Older Midori builds inherited uBOL's persisted badge preference. Enable
    // the count once during migration, then continue respecting user changes.
    if ( (rulesetConfig.midoriBadgeVersion ?? 0) < 1 ) {
        rulesetConfig.showBlockedCount = true;
        rulesetConfig.midoriBadgeVersion = 1;
        await saveRulesetConfig();
    }
    ...
    await browser.action.setBadgeBackgroundColor({ color: '#0f9f6e' });
